...
public class MealRecordService {
    ...
    public List<MealRecord> getRecordsByDate(Long userId, Date date) {
        System.out.println("=== 查询记录调试信息 ===");
        System.out.println("用户ID: " + userId);
        System.out.println("查询日期: " + date);
        System.out.println("日期类型: " + date.getClass().getName());
        System.out.println("日期值: " + date.toString());

        List<MealRecord> records = mealRecordMapper.findByUserAndDate(userId, date);

        System.out.println("查询到的记录数: " + records.size());
        if (!records.isEmpty()) {
            System.out.println("第一条记录日期: " + records.get(0).getDate());
            System.out.println("最后一条记录日期: " + records.get(records.size()-1).getDate());
        }
    ...
    public Map<String, Object> analyzeDailyRecords(Long userId, Date date) {
        ...
        if (records == null || records.isEmpty()) {
            ...
            throw new RuntimeException("该日期没有饮食记录，请先添加饮食记录");
        }

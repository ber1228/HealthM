...
public class PlanService {
    ...
    private List<DailyPlan> generateMeal(Long userId, Date date, String mealType, BigDecimal targetCalories) {
        List<DailyPlan> plans = new ArrayList<>();
        List<Food> foods = foodMapper.findAll();
        
        // 根据健康目标选择食物
        List<Food> selectedFoods = selectFoodsByGoal(foods, targetCalories);
        ...
        for (int i = 0; i < selectedFoods.size() && i < calorieRatios.length; i++) {
            ...
            if (amount.compareTo(new BigDecimal("50")) < 0) {
                amount = new BigDecimal("50"); // 最小50g
            }
    ...
    private DailyPlan generateSnack(Long userId, Date date, String mealType, BigDecimal targetCalories) {
        List<Food> snacks = foodMapper.findByCategory("水果");
    ...
    private List<Food> selectFoodsByGoal(List<Food> foods, BigDecimal targetCalories) {
        List<Food> selected = new ArrayList<>();
        Random random = new Random();
        
        // 确保营养均衡：碳水化合物50-60%，蛋白质15-20%，脂肪20-30%
        
        // 选择谷物（碳水化合物）
        List<Food> grains = foods.stream().filter(f -> "谷物".equals(f.getCategory())).collect(Collectors.toList());
        if (!grains.isEmpty()) {
            selected.add(grains.get(random.nextInt(grains.size())));
        }
        ...
        if (!proteins.isEmpty()) {
            selected.add(proteins.get(random.nextInt(proteins.size())));
        }
        
        // 选择蔬菜（维生素和矿物质）
        List<Food> vegetables = foods.stream().filter(f -> "蔬菜".equals(f.getCategory())).collect(Collectors.toList());
        if (!vegetables.isEmpty()) {
            selected.add(vegetables.get(random.nextInt(vegetables.size())));
        }
        
        // 选择健康脂肪
        List<Food> fats = foods.stream().filter(f -> 
            "坚果".equals(f.getCategory()) || "油脂".equals(f.getCategory())).collect(Collectors.toList());
        if (!fats.isEmpty() && targetCalories.compareTo(new BigDecimal("400")) > 0) {
            selected.add(fats.get(random.nextInt(fats.size())));
        }
